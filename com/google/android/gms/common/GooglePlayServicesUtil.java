...
public final class GooglePlayServicesUtil extends zze {
    ...
    private static void zza(int i, Context context, String str) {
        ...
        if (zzg == null) {
            zzg = resources.getString(C0233R.string.common_google_play_services_notification_ticker);
        }
        ...
        if (zzmu.zzaw(context)) {
            zzx.zzab(zzne.zzsh());
            build = new Notification.Builder(context).setSmallIcon(C0233R.drawable.common_ic_googleplayservices).setPriority(2).setAutoCancel(true).setStyle(new BigTextStyle().bigText(zzg + " " + zzc)).addAction(C0233R.drawable.common_full_open_on_phone, resources.getString(C0233R.string.common_open_on_phone), zza).build();
        } else {
            CharSequence string = resources.getString(C0233R.string.common_google_play_services_notification_ticker);
            if (zzne.zzsd()) {
                Notification build2;
