...
class zzcl implements Runnable {
    ...
    private boolean zzGX() {
        ...
        zzbg.m16v("...no network connectivity");
        return false;
    }

    private void zzGY() {
        if (zzGX()) {
            zzbg.m16v("Start loading resource from network ...");
            String zzGZ = zzGZ();
            ...
            try {
                ...
                try {
                    ...
                    zzbg.m16v("Successfully loaded supplemented resource: " + zzd);
                    if (zzd.zzju == null && zzd.zzjt.length == 0) {
                        zzbg.m16v("No change for container: " + this.zzbhM);
                    }
                    this.zzbkg.zzI(zzd);
                    zzbg.m16v("Load resource from network finished.");
                } catch (Throwable e) {
