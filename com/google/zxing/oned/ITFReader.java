...
public final class ITFReader extends OneDReader {
    ...
    private static final int f22N = 1;
    static final int[][] PATTERNS;
    private static final int[] START_PATTERN;
    private static final int f23W = 3;
    ...
    static {
        ...
        START_PATTERN = new int[]{f22N, f22N, f22N, f22N};
        END_PATTERN_REVERSED = new int[]{f22N, f22N, f23W};
        PATTERNS = new int[][]{new int[]{f22N, f22N, f23W, f23W, f22N}, new int[]{f23W, f22N, f22N, f22N, f23W}, new int[]{f22N, f23W, f22N, f22N, f23W}, new int[]{f23W, f23W, f22N, f22N, f22N}, new int[]{f22N, f22N, f23W, f22N, f23W}, new int[]{f23W, f22N, f23W, f22N, f22N}, new int[]{f22N, f23W, f23W, f22N, f22N}, new int[]{f22N, f22N, f22N, f23W, f23W}, new int[]{f23W, f22N, f22N, f23W, f22N}, new int[]{f22N, f23W, f22N, f23W, f22N}};
    }
    ...
    public Result decodeRow(int rowNumber, BitArray row, Map<DecodeHintType, ?> hints) throws FormatException, NotFoundException {
        ...
        decodeMiddle(row, startRange[f22N], endRange[0], result);
        ...
        if (lengthOK) {
            ...
            resultPointArr[0] = new ResultPoint((float) startRange[f22N], (float) rowNumber);
            resultPointArr[f22N] = new ResultPoint((float) endRange[0], (float) rowNumber);
    ...
    int[] decodeStart(BitArray row) throws NotFoundException {
        ...
        this.narrowLineWidth = (startPattern[f22N] - startPattern[0]) / 4;
    ...
    int[] decodeEnd(BitArray row) throws NotFoundException {
        ...
        try {
            ...
            endPattern[0] = row.getSize() - endPattern[f22N];
            endPattern[f22N] = row.getSize() - temp;
            return endPattern;
        } finally {
            row.reverse();
        }
    ...
    private static int[] findGuardPattern(BitArray row, int rowOffset, int[] pattern) throws NotFoundException {
        ...
        for (int x = rowOffset; x < width; x += f22N) {
            if ((row.get(x) ^ isWhite) != 0) {
                counters[counterPosition] = counters[counterPosition] + f22N;
            } else {
                if (counterPosition != patternLength - 1) {
                    counterPosition += f22N;
                } else if (OneDReader.patternMatchVariance(counters, pattern, MAX_INDIVIDUAL_VARIANCE) < MAX_AVG_VARIANCE) {
                    return new int[]{patternStart, x};
                } else {
                    patternStart += counters[0] + counters[f22N];
                    System.arraycopy(counters, 2, counters, 0, patternLength - 2);
                    counters[patternLength - 2] = 0;
                    counters[patternLength - 1] = 0;
                    counterPosition--;
                }
                counters[counterPosition] = f22N;
                if (isWhite) {
                    isWhite = false;
                } else {
                    isWhite = true;
