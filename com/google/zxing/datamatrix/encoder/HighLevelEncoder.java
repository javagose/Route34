    ...
    static int lookAheadTest(CharSequence msg, int startpos, int currentMode) {
        ...
        while (startpos + charsProcessed != msg.length()) {
            ...
            if (isDigit(c)) {
                ...
                charCounts[ASCII_ENCODATION] = (float) ((int) Math.ceil((double) charCounts[ASCII_ENCODATION]));
                charCounts[ASCII_ENCODATION] = charCounts[ASCII_ENCODATION] + DefaultRetryPolicy.DEFAULT_BACKOFF_MULT;
            }
            if (isNativeC40(c)) {
            ...
            if (isSpecialB256(c)) {
                ...
                charCounts[BASE256_ENCODATION] = charCounts[BASE256_ENCODATION] + DefaultRetryPolicy.DEFAULT_BACKOFF_MULT;
            }
            if (charsProcessed >= EDIFACT_ENCODATION) {
                intCharCounts = new int[6];
                mins = new byte[6];
                findMinimums(charCounts, intCharCounts, Strategy.TTL_SECONDS_INFINITE, mins);
        ...
        mins = new byte[6];
        intCharCounts = new int[6];
        int min = findMinimums(charCounts, intCharCounts, Strategy.TTL_SECONDS_INFINITE, mins);
        minCount = getMinimumCount(mins);
