...
public class DecoderThread {
    ...
    class C06181 implements Callback {
        C06181() {
        }

        public boolean handleMessage(Message message) {
            if (message.what == C0599R.id.zxing_decode) {
    ...
    class C09682 implements PreviewCallback {
        C09682() {
        }
        ...
        public void onPreview(SourceData sourceData) {
            synchronized (DecoderThread.this.LOCK) {
                if (DecoderThread.this.running) {
                    DecoderThread.this.handler.obtainMessage(C0599R.id.zxing_decode, sourceData).sendToTarget();
                }
            }
        }
    }
    ...
    public DecoderThread(CameraInstance cameraInstance, Decoder decoder, Handler resultHandler) {
        ...
        this.callback = new C06181();
        this.previewCallback = new C09682();
    ...
    private void decode(SourceData sourceData) {
        ...
        if (rawResult != null) {
            ...
            if (this.resultHandler != null) {
                Message message = Message.obtain(this.resultHandler, C0599R.id.zxing_decode_succeeded, new BarcodeResult(rawResult, sourceData));
                message.setData(new Bundle());
                message.sendToTarget();
            }
        } else if (this.resultHandler != null) {
            Message.obtain(this.resultHandler, C0599R.id.zxing_decode_failed).sendToTarget();
        }
        if (this.resultHandler != null) {
            Message.obtain(this.resultHandler, C0599R.id.zxing_possible_result_points, this.decoder.getPossibleResultPoints()).sendToTarget();
        }
        requestNextPreview();
    }
}
