...
public class CompoundBarcodeView extends FrameLayout {
    ...
    private void initialize(AttributeSet attrs) {
        TypedArray attributes = getContext().obtainStyledAttributes(attrs, C0599R.styleable.zxing_view);
        int scannerLayout = attributes.getResourceId(C0599R.styleable.zxing_view_zxing_scanner_layout, C0599R.layout.zxing_barcode_scanner);
        attributes.recycle();
        inflate(getContext(), scannerLayout, this);
        this.barcodeView = (BarcodeView) findViewById(C0599R.id.zxing_barcode_surface);
        if (this.barcodeView == null) {
            throw new IllegalArgumentException("There is no a com.journeyapps.barcodescanner.BarcodeView on provided layout with the id \"zxing_barcode_surface\".");
        }
        this.barcodeView.initializeAttributes(attrs);
        this.viewFinder = (ViewfinderView) findViewById(C0599R.id.zxing_viewfinder_view);
        if (this.viewFinder == null) {
            throw new IllegalArgumentException("There is no a com.journeyapps.barcodescanner.ViewfinderView on provided layout with the id \"zxing_viewfinder_view\".");
        }
        this.viewFinder.setCameraPreview(this.barcodeView);
        this.statusView = (TextView) findViewById(C0599R.id.zxing_status_view);
    }
    ...
    public BarcodeView getBarcodeView() {
        return (BarcodeView) findViewById(C0599R.id.zxing_barcode_surface);
    }
